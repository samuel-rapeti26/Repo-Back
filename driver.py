...
def get_dictionary():
    ...
    abb = pd.read_csv(filepath_or_buffer='./Abbrev_Exceptions_DrugName.txt', header=None)
    accepted_abbrev = list(abb[0].values)
...
def prepareData(inputData):
        ...
        def extract_hpi(text):
            for i in range(len(text)):
                for j in range(i+1, len(text)):
                    if re.sub(r'[^\w\s]', '',text[i]).strip().lower() in ["hpi", "history of present illness"] and re.sub(r'[^\w\s]', '', text[j]).strip().lower() in ["physical examination", "physical exam", "review of systems", "objective", "assessment"]:
                        return text[i+1:j]
                    elif re.sub(r'[^\w\s]', '',text[i]).strip().lower() in ["hpi", "history of present illness"] and (re.sub(r'[^\w\s]', '',text[j]).strip().lower()).startswith("physical exam"):
                        return text[i+1:j]
            
        hpi = extract_hpi(inputData)
        
        if hpi != None:
            inputData1 = [i for i in hpi if i!='']
        else:
            inputData1 = []
    ...
    #     print("ORIGINAL", inp)
    #     print("^"*100)
    #     print("LATER", inputData1)
        
        key = inputData
        val = [i+1 for i in range(len(inputData))]
        d = dict(zip(val,key))
        ...
        fileObject = pd.read_csv("./Wrong_pronouns.txt", header=None, sep="\t")
        data = fileObject[0][0]
        ...
        ### 3. Tense ###
        # function to extract HPI
        input_ann=pd.DataFrame(columns=['doc_id','sentence_id','token_id','token','upos','xpos', 'feats'])
        for t in range(len(inputData1)):
            doc = nlp(inputData1[t])
            for token in doc:
                input_ann.loc[len(input_ann)]=['doc'+ str(t+1),t+1,len(input_ann)+1,token.text, token.pos_, token.tag_, token.morph]
        input_ann['start']=''
        input_ann['end']=''
        for t in range(len(inputData1)):
            ...
            startFrom=0
            for x in para_rows:
                if input_ann.loc[x,'upos'] not in ['PUNCT','NUM','SYM']:
                    if re.search(re.escape(input_ann.loc[x,'token']),inputData1[t][startFrom:]):
                        pos=re.search(re.escape(input_ann.loc[x,'token']),inputData1[t][startFrom:]).span()
        ...
        for para_id in para_ids:
            paranum = int(re.findall('\\d+',para_id)[0])-1
                ...
                if len(aux_verbs)>0:
                    ...
                    if aux_pres_rows.any():
                        ...
                        for i in range(len(verb)):
                            ...
                            if not re.search("'t|n't|'ve",inputData1[par-1][ed:ed+2].lower()) and re.search("\\w",inputData1[par-1][ed:ed+2]):
                                suggest[i]=suggest[i] + ""
                            if verb.str.lower()[i]=='may':
                                ...
                                checker=inputData1[par-1][(st-8):(ed+8)]
                        ...
                        verbDetected=pd.concat([verbDetected, pd.DataFrame(data={'verb':verb,'suggest':suggest,'start':start,'end':end})], axis=0, ignore_index=True) 
                if len(main_verbs)>0:
                ...
                if len(verbDetected)>0:
                    for i in range(len(verbDetected)):
                        if re.search('follow',verbDetected['verb'][i],flags=re.IGNORECASE):
                            nextWords= inputData1[int(re.findall('\\d+',para_id)[0])-1][verbDetected['end'][i]+1:verbDetected['end'][i]+20]
                        ...
                        paraGraphNum=paraGraphNum + [key for key, val in d.items() if inputData1[paranum]==val]
        ...
        input_ann1=pd.DataFrame(columns=['doc_id','sentence_id','token_id','token','upos','xpos', 'feats'])
        for t in range(len(inputData)):
            doc = nlp(inputData[t])
            for token in doc:
                input_ann1.loc[len(input_ann1)]=['doc'+ str(t+1),t+1,len(input_ann1)+1,token.text, token.pos_, token.tag_, token.morph]
        input_ann1['start']=''
        input_ann1['end']=''
        for t in range(len(inputData)):
            para_rows=input_ann1[input_ann1['doc_id'].isin(['doc' + str(t+1)])].index
            startFrom=0
            for x in para_rows:
                if input_ann1.loc[x,'upos'] not in ['PUNCT','NUM','SYM']:
                    if re.search(re.escape(input_ann1.loc[x,'token']),inputData[t][startFrom:]):
                        pos=re.search(re.escape(input_ann1.loc[x,'token']),inputData[t][startFrom:]).span()
                        pos=tuple(map(lambda x:x+startFrom,pos))
                        startFrom=pos[1]
                        input_ann1.loc[x,'start']=pos[0]
                        input_ann1.loc[x,'end']=pos[1]

        
        input_ann_NNP=input_ann1.loc[input_ann1['xpos']=='NNP',['doc_id','token','start','end']]
        input_ann_NNP['doc_id']=[int(re.findall("\\d+",i)[0]) for i in input_ann_NNP['doc_id'] if re.search("\\d+",i)]
        def func(d):
            rows=input_ann1.loc[(input_ann1['xpos']=='NNP') & (input_ann1['doc_id']==d),'token'].tolist()
            return rows
                            ...
                            if len(tempWord)>0:
                                ...
                                for k in wordIndex:
                                    ...
                                    if flag:
                                        ...
                                        #paraGraphNum=paraGraphNum + [d[inputData[i]]]
                                            ...
                                            if len(additionalSpellSuggestion[additionalSpellSuggestion['key']==tempWord]) != 0:
                                                if len(unitSpellSuggestion[unitSpellSuggestion['key']==tempWord])!=0:
                                                    if re.search("(?<![A-Za-z0-9.])(\\d*[.]{0,1}\\d+)(?![A-Za-z0-9.])",prevWord):
                                                        ...
                                                        suggest = [i for n, i in enumerate(suggest) if i not in suggest[:n]]
                                                
                                                    else:
                                                        suggest = list(additionalSpellSuggestion[additionalSpellSuggestion['key']==tempWord]['val'].values) + \
                                                                        suggest + list(unitSpellSuggestion[unitSpellSuggestion['key']==tempWord]['val'].values)
                                                        suggest = [i for n, i in enumerate(suggest) if i not in suggest[:n]]
                                                ...
                                                if len(unitSpellSuggestion[unitSpellSuggestion['key']==tempWord])!=0:
                                                    if re.search("(?<![A-Za-z0-9.])(\\d*[.]{0,1}\\d+)(?![A-Za-z0-9.])",prevWord):
                                                        suggest = list(unitSpellSuggestion[unitSpellSuggestion['key']==tempWord]['val'].values) + suggest
                                                        suggest = [i for n, i in enumerate(suggest) if i not in suggest[:n]]
                                                
                                                    else:
                                                        suggest = suggest + list(unitSpellSuggestion[unitSpellSuggestion['key']==tempWord]['val'].values)
                                                        suggest = [i for n, i in enumerate(suggest) if i not in suggest[:n]]
        ...
        if len(replacedWordIndex)>0:
            for t in range(len(replacedWordIndex)):
                if len(replacedWordIndex[t])!=0:
                    for j in range(math.ceil(len(replacedWordIndex[t])/2)):
                        ...
                        for s in tempWord:
                            ...
                            if sugg is not None:
                                ...
                                #paraGraphNum=paraGraphNum + [d[inputData[t]]]
        ...
        for j in range(len(inputData)):
            ...
            for k in replacedWord:
                paraGraphNum=paraGraphNum + [j+1]
                #paraGraphNum=paraGraphNum + [d[inputData[j]]]
                wordDetected=wordDetected + [k.group(0)]
                suggest=' '
                suggestedWord=suggestedWord + [suggest]
        ...
        for p in range(len(inputData)):
            ...
            for s in replacedWord:
                ...
                #paraGraphNum=paraGraphNum + [d[inputData[p]]]
        ...
        for p in range(len(index_of_occ)):
            if len(index_of_occ[p]) > 0:                   
                for i in range(len(index_of_occ[p])):
                    ...
                    if ABB2 in accepted_abbrev:     
                        continue
                    h_GB=Hunspell('en_GB')
                    h_en=Hunspell('en_US')
                    if h_GB.spell(ABB2) and h_en.spell(ABB2):
                        continue
        ...
        # identify past tense from PLAN
        # function to extract PLAN
        def extract_plan(text):
            for i in range(len(text)):
                for j in range(i+1, len(text)):
                    if re.sub(r'[^\w\s]', '',text[i]).strip().lower() in ["plan", "plans"] and re.sub(r'[^\w\s]', '',text[j]).strip().lower() in ["attestation", "followup followup", "followup", "orders", "order"]:
                        return text[i+1:j]
                    elif re.sub(r'[^\w\s]', '',text[i]).strip().lower() in ["plan", "plans"] and (re.sub(r'[^\w\s]', '',text[j]).strip().lower()).startswith("documentation services"):
                        return text[i+1:j]
            
        plan = extract_plan(inputData)
        if plan != None:
            inputData2 = [i for i in plan if i!='']
        else:
            inputData2 = []
        
        # check POS tag of text
        def tags(text):
            tag_list = []
            for sen in text:
                for token in nlp(sen):
                    if "VBN" in token.tag_ or "VBD" in token.tag_:
                        tag_list.append(token.text)
                    actual=["was", "were", "had", "would", "should", "might", "could"]
                    if token.text in actual and token.text not in tag_list:
                        tag_list.append(token.text)
            return tag_list

        past_words = tags(inputData2)
        if past_words != []:
            for j in range(len(inputData2)):
                word = re.finditer("|".join(past_words), inputData2[j])
                for w in word:
                    paraGraphNum=paraGraphNum + [key for key, val in d.items() if inputData2[j] in val]
                    wordDetected=wordDetected + [w.group(0)]
                    suggest=''
                    suggestedWord=suggestedWord + [suggest]
                    errorCode=errorCode + ['Past Tense used']
                    category=category + ['Optional']
                    startPos=startPos + [w.start()]
                    endPos=endPos + [w.end()]
                    operation=operation + ['tense']
                    frontendAction=frontendAction + ['No action']
        ...
        if inputData1 != []:
            sen = [str(i) for i in nlp(inputData1[0]).sents]
            if sen != []:
                sen1_tag = [token.tag_ for token in nlp(sen[0])]
                if "NNP" in sen1_tag:
                    if [key for key, val in d.items() if sen[0] in val]!= []:
                        para = [key for key, val in d.items() if sen[0] in val][0] 
                        hpi_index = wordTable.loc[(wordTable['ParagraphNum'] == para) & (wordTable['StartPos'] <= len(sen[0])) & (wordTable['ErrorType'] == "Incorrect Tense used")].index
                #print(wordTable.loc[(wordTable['ParagraphNum'] == para) & (wordTable['StartPos'] <= len(sen[0])) & (wordTable['ErrorType'] == "Incorrect Tense used")])
                        wordTable = wordTable.drop(hpi_index)
        ...
        if len(wordTable) == 0:
            ...
            wordTable = wordTable.drop_duplicates()
