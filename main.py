...
requestHeaders = ['Content-Type', 'Strict-Transport-Security']
...
@app.route('/temptable', methods=["POST"])
@cross_origin(origin='localhost',headers=requestHeaders)
# jwt_required() will authorize for given JWT
@jwt_required()
def modifytable():
...
@app.route('/viewdict', methods=["POST"])
@cross_origin(origin='localhost',headers=requestHeaders)
# jwt_required() will authorize for given JWT
@jwt_required()
def viewdict():
...
if __name__ == "__main__":
    ...
    serve(app, host="0.0.0.0", port=2000, url_scheme='https')